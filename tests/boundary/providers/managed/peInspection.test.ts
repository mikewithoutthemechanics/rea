...
import {
  ...
  type PeDebugEntry,
  type PeTlsCallback,
} from "../../../../src/domain/peInspection.js";
...
describe("PE/COFF static inspection", () => {
  ...
  it("uses all exported types", () => {
    const section: PeSection = validManifest.sections[0]!;
    expect(section.name).toBe(".text");
    const imp: PeImport = validManifest.imports[0]!;
    expect(imp.dll).toBe("KERNEL32.DLL");
    const exp: PeExport = validManifest.exports[0]!;
    expect(exp.name).toBe("DllMain");
    const res: PeResource = validManifest.resources[0]!;
    expect(res.type).toBe("RT_VERSION");
    const dbg: PeDebugEntry = validManifest.debug_entries[0]!;
    expect(dbg.type).toBe("CODEVIEW");
    const tls: PeTlsCallback = validManifest.tls_callbacks[0]!;
    expect(tls.index).toBe(0);
  });
});
