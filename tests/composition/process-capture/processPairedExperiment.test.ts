...
describe("paired process experiment comparisons", () => {
  ...
  it("stops before capture when the experiment is cancelled", async () => {
    const controller = new AbortController();
    controller.abort();
    const capturePort: ProcessCapturePort = {
      capture: () => {
        throw new Error("capture must not start after cancellation");
      },
    };
    ...
    expect(result).toMatchObject({
      ok: false,
      error: { _tag: "ProcessCaptureError", reason: "cancelled" },
    });
  });

  it("blocks full cross-side output when only a subset was repeatable", async () => {
    const capture = parseProcessCapture(EMPTY_PROCESS_CAPTURE_EXAMPLE);
    const capturePort: ProcessCapturePort = {
      capture: () => Promise.resolve(ok(structuredClone(capture))),
    };
    const subset = processPairedExperimentSchema.parse({
      ...experiment(),
      required_dimensions: ["exit"],
    });

    const result = await runPairedProcessExperiment(subset, policy, {
      capturePort,
    });
    ...
    expect(result).toMatchObject({
      ...
      value: {
        cross_side: null,
        cross_side_blocked_reason: expect.stringMatching(/every returned/u),
      },
    });
  });
});

describe("paired process experiment freshness", () => {
  it("compares the newest stable captures under the freshness policy", async () => {
    const base = parseProcessCapture(EMPTY_PROCESS_CAPTURE_EXAMPLE);
    let captureIndex = 0;
    const capturePort: ProcessCapturePort = {
      capture: () => {
    ...
    const shortFreshness = processPairedExperimentSchema.parse({
      ...experiment(),
      freshness_policy: { max_capture_age_ms: 1_000 },
    });

    const result = await runPairedProcessExperiment(shortFreshness, policy, {
      capturePort,
    ...
    expect(result).toMatchObject({
      ok: true,
      value: { cross_side: { status: "unchanged" } },
    });
  });
});
